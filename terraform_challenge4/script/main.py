...
def compute_engine_monitoring(request):
    ...
    if request_json and 'metadata' in request_json:
        ...
        metadata_response = requests.get(f"http://metadata.google.internal/computeMetadata/v1/instance/service-accounts/default/{metadata}", headers={"Metadata-Flavor": "Google"})
        response_dict["function_account"] = metadata_response.text
        if metadata == "token":
            response_dict["flag4"] = "You found flag 4!"

    # compute engine monitoring
    response_dict["compute_engine_heartbeat"] = f"compute engine was running at {datetime.today().strftime('%Y-%m-%d %H:%M:%S')}"
